  ...
  ngOnInit() {
    this.route.queryParams.subscribe((params) => {
      ...
      if (this.status == 'success') {
        ...
        const paymentData = localStorage.getItem('bb_payment_data') ?? null;
        this.responseData =
          paymentData !== null ? JSON.parse(paymentData) : null;
        
        if(paymentData !== null){
            const userId = this.paymentData.userId;
            ...
            if(paymentData.sessionId) documentData.stripeSessionId = paymentData.sessionId;
        
            if(paymentData.subscriptionId) documentData.stripeSubscriptionId = paymentData.subscriptionId;
